...
一个轻量级、高性能的 Unity 视觉特效管理系统，通过智能对象池优化 VFX 播放性能。
...
VFXSystem.Instance.PlayAt(
    longEffectPrefab,
    position,
    durationOverride: 10f  // 强制 10 秒时长
);
```
...
**可能原因：**
- 预制体不包含 ParticleSystem 组件
- VFXPool 未初始化（检查 `VFXPool.Instance`）
- 时长计算失败（检查控制台警告）
...
**解决方案：**
```csharp
// 1. 在 VFXPool 配置中检查池大小
// 2. 检查控制台的集合检查警告
// 3. 确保所有 VFX 都通过 VFXSystem API
```

## 📋 系统要求

- **Unity 版本**：Unity 2020.3 或更高（使用 `FindFirstObjectByType`）
- **依赖**：无（全部包含）
- **命名空间**：`CDTU.Utils`（包含在包内）
